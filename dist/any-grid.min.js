...
},Outlayer.prototype.needsResizeLayout=function(){var size=getSize(this.element),hasSizes=this.size&&size;return hasSizes&&size.innerWidth!==this.size.innerWidth},Outlayer.prototype.addItems=function(elems){var items=this._itemize(elems);return items.length&&(this.items=this.items.concat(items)),items},Outlayer.prototype.appended=function(elems){var items=this.addItems(elems);items.length&&(this.layoutItems(items,!0),this.reveal(items))},Outlayer.prototype.prepended=function(elems){var items=this._itemize(elems);if(items.length){var previousItems=this.items.slice(0);this.items=items.concat(previousItems),this._resetLayout(),this._manageStamps(),this.layoutItems(items,!0),this.reveal(items),this.layoutItems(previousItems)}},Outlayer.prototype.reveal=function(items){this._emitCompleteOnItems("reveal",items);for(var len=items&&items.length,i=0;len&&len>i;i++){var item=items[i];item.reveal()}},Outlayer.prototype.hide=function(items){this._emitCompleteOnItems("hide",items);for(var len=items&&items.length,i=0;len&&len>i;i++){var item=items[i];item.hide()}},Outlayer.prototype.revealItemElements=function(elems){var items=this.getItems(elems);this.reveal(items)},Outlayer.prototype.hideItemElements=function(elems){var items=this.getItems(elems);this.hide(items)},Outlayer.prototype.getItem=function(elem){for(var i=0,len=this.items.length;len>i;i++){var item=this.items[i];if(item.element===elem)return item}},Outlayer.prototype.getItems=function(elems){elems=utils.makeArray(elems);for(var items=[],i=0,len=elems.length;len>i;i++){var elem=elems[i],item=this.getItem(elem);item&&items.push(item)}return items},Outlayer.prototype.remove=function(elems){var removeItems=this.getItems(elems);if(this._emitCompleteOnItems("remove",removeItems),removeItems&&removeItems.length)for(var i=0,len=removeItems.length;len>i;i++){var item=removeItems[i];item.remove(),utils.removeFrom(this.items,item)}},Outlayer.prototype.destroy=function(){var style=this.element.style;style.height="",style.position="",style.width="";for(var i=0,len=this.items.length;len>i;i++){var item=this.items[i];item.destroy()}this.unbindResize();var id=this.element.outlayerGUID;delete instances[id],delete this.element.outlayerGUID,jQuery&&jQuery.removeData(this.element,this.constructor.namespace)},Outlayer.data=function(elem){elem=utils.getQueryElement(elem);var id=elem&&elem.outlayerGUID;return id&&instances[id]},Outlayer.create=function(namespace,options){function Layout(){Outlayer.apply(this,arguments)}return Object.create?Layout.prototype=Object.create(Outlayer.prototype):utils.extend(Layout.prototype,Outlayer.prototype),Layout.prototype.constructor=Layout,Layout.defaults=utils.extend({},Outlayer.defaults),utils.extend(Layout.defaults,options),Layout.prototype.settings={},Layout.namespace=namespace,Layout.data=Outlayer.data,Layout.Item=function(){Item.apply(this,arguments)},Layout.Item.prototype=new Item,utils.htmlInit(Layout,namespace),jQuery&&jQuery.bridget&&jQuery.bridget(namespace,Layout),Layout},Outlayer.Item=Item,Outlayer}),function(window,factory){"use strict";window.AnyGrid=factory(window.Outlayer)}(window,function(Outlayer){"use strict";var AnyGrid=Outlayer.create("anyGrid",{masonry:!1,orderMasonry:!0,stacked:!1,perRow:{xxs:1,xs:1,sm:2,md:3,lg:4,xl:5,xxl:6},isLayoutInstant:!0,hiddenStyle:{opacity:0},visibleStyle:{opacity:1},adjustGutter:!1,gutter:!1,removeVerticalGutters:!1});return AnyGrid.prototype.getBreakPoint=function(){return this.breakPoint},AnyGrid.prototype.getPerRow=function(){return this.perRow},AnyGrid.prototype.resize=function(){this.isResizeBound&&this.needsResizeLayout()&&(this._setUp(),this.emitEvent("resize",[this]),this.layout(),this.emitEvent("resized",[this]))},AnyGrid.prototype._setUp=function(){this.getSize(),this.containerWidth=this.size.outerWidth,this.containerWidth>=1500?(this.perRow=this.options.perRow.xxl,this.breakPoint="xxl"):this.containerWidth>=1250?(this.perRow=this.options.perRow.xl,this.breakPoint="xl"):this.containerWidth>=1e3?(this.perRow=this.options.perRow.lg,this.breakPoint="lg"):this.containerWidth>=750?(this.perRow=this.options.perRow.md,this.breakPoint="md"):this.containerWidth>=500?(this.perRow=this.options.perRow.sm,this.breakPoint="sm"):this.containerWidth>=250?(this.perRow=this.options.perRow.xs,this.breakPoint="xs"):(this.perRow=this.options.perRow.xxs,this.breakPoint="xxs"),this.perRow||(this.perRow=parseInt(this.options.perRow)),this.verticalPadding=this.options.gutter?this.options.gutter:this.items.length?getSize(this.items[0].element).paddingTop:0;var measureContainerWidth=this.containerWidth;if(this.options.adjustGutter&&(this.items.length||this.options.gutter)){var padding=this.options.gutter?this.options.gutter:getSize(this.items[0].element).paddingLeft;this.element.parentNode.style.marginLeft=-1*padding+"px",measureContainerWidth=this.containerWidth+2*padding}this.columnWidth=measureContainerWidth/this.perRow,this.cols=Math.floor(measureContainerWidth/this.columnWidth),this.cols=Math.max(this.cols,1),this.columns=[],this.rows={};for(var i=0;i<this.cols;i++)this.columns[i]=0;this.maxHeight=0,this.itemIndex=0},AnyGrid.prototype._create=function(){this.reloadItems(),this.element.style.position="relative",this.options.isResizeBound&&this.bindResize(),this._setUp()},AnyGrid.prototype._resetLayout=function(){this._setUp()},AnyGrid.prototype._getItemLayoutPosition=function(item){item.element.style.width=this.columnWidth+"px",item.getSize();var column=this.itemIndex%this.cols,x=column*this.columnWidth,y=this.columns[column],itemHeight=this.options.itemHeight?this.options.itemHeight:item.size.height;this.rowsCount=this.items.length/this.cols;var row=Math.floor(this.itemIndex/this.cols)+1;if(this.options.stacked&&(column=Math.floor(this.itemIndex/this.rowsCount),row=Math.abs(column*this.rowsCount-this.itemIndex)+1),this.options.removeVerticalGutters&&(1===row&&item.size.paddingTop&&(itemHeight-=this.verticalPadding),row===this.rowsCount&&item.size.paddingBottom&&(itemHeight-=this.verticalPadding)),this.options.masonry)if(this.options.orderMasonry)this.columns[column]=this.columns[column]+itemHeight,this.maxHeight=Math.max(this.maxHeight,this.columns[column]);else{var minimumY=Math.min.apply(Math,this.columns),shortColIndex=this.columns.indexOf(minimumY),x=this.columnWidth*shortColIndex,y=minimumY;this.columns[shortColIndex]=minimumY+itemHeight,this.maxHeight=Math.max.apply(Math,this.columns)}else if(this.options.stacked)x=column*this.columnWidth,y=this.columns[column],this.columns[column]=this.columns[column]+itemHeight,this.maxHeight=Math.max(this.maxHeight,this.columns[column]);else{var firstColumn=row>1;this.columns[column]=firstColumn?this.rows[row-1].maxHeight+itemHeight:this.columns[column]+itemHeight,firstColumn?(y=this.rows[row-1].maxHeight,this.maxHeight=Math.max(this.maxHeight,y+itemHeight)):this.maxHeight=Math.max(this.maxHeight,itemHeight)}return this.rows[row]=this.rows[row]||{},(!this.rows[row].maxHeight||this.columns[column]>this.rows[row].maxHeight)&&(this.rows[row].maxHeight=this.columns[column]),(!this.rows[row].height||itemHeight>this.rows[row].height)&&(this.rows[row].height=itemHeight),this.itemIndex++,{row:row,x:x,y:y}},AnyGrid.prototype._processLayoutQueue=function(queue){for(var i=0,len=queue.length;len>i;i++){var obj=queue[i];this._positionItem(obj.item,obj.x,obj.y,obj.isInstant,obj.row)}},AnyGrid.prototype._positionItem=function(item,x,y,isInstant,row){isInstant?item.goTo(x,y):item.moveTo(x,y,this.options.stacked?!0:!1),this.options.removeVerticalGutters&&(item.element.style.padding=this.verticalPadding+"px",1===row&&(item.element.style.paddingTop="0"),row===this.rowsCount&&(item.element.style.paddingBottom="0"))},AnyGrid.prototype._postLayout=function(){this.resizeItems(),this.resizeContainer()},AnyGrid.prototype.resizeItems=function(){for(var i=0;i<this.items.length;i++)this.items[i].element.style.width=this.columnWidth+"px"},AnyGrid.prototype._getContainerSize=function(){return{height:this.maxHeight}},AnyGrid}),function(window,factory){"use strict";window.AnyGrid=window.AnyGrid||{},window.AnyGrid.Item=factory(window.Outlayer)}(window,function(Outlayer){"use strict";function Item(){Outlayer.Item.apply(this,arguments)}function toDashedAll(str){return str.replace(/([A-Z])/g,function($1){return"-"+$1.toLowerCase()})}var transitionProperty=getStyleProperty("transition"),transformProperty=getStyleProperty("transform"),supportsCSS3=transitionProperty&&transformProperty,transitionEndEvent=(!!getStyleProperty("perspective"),{WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"otransitionend",transition:"transitionend"}[transitionProperty]),prefixableProperties=["transform","transition","transitionDuration","transitionProperty"],vendorProperties=function(){for(var cache={},i=0,len=prefixableProperties.length;len>i;i++){var prop=prefixableProperties[i],supportedProp=getStyleProperty(prop);supportedProp&&supportedProp!==prop&&(cache[prop]=supportedProp)}return cache}();Item.prototype=new Outlayer.Item;var transitionProps="opacity, height, width, padding,"+toDashedAll(vendorProperties.transform||"transform");return Item.prototype.moveTo=function(x,y,force){if(!supportsCSS3)return void this.goTo(x,y);this.getPosition();var curX=this.position.x,curY=this.position.y,compareX=parseInt(x,10),compareY=parseInt(y,10),didNotMove=compareX===this.position.x&&compareY===this.position.y;if(this.setPosition(x,y),!force&&didNotMove&&!this.isTransitioning)return void this.layoutPosition();var transX=x-curX,transY=y-curY,transitionStyle={};transitionStyle.transform=this.getTranslate(transX,transY),this.transition({to:transitionStyle,onTransitionEnd:{transform:this.layoutPosition},isCleaning:!0})},Item.prototype.enableTransition=function(){this.isTransitioning||(this.css({transitionProperty:transitionProps,transitionDuration:this.layout.options.transitionDuration}),this.element.addEventListener(transitionEndEvent,this,!1))},Item});